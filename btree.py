...
class BTreeBlock(Block):
	...
	# The following roughly implements the algorithm shown in Figures 14.16 and 14.17 (7th edition)
	...
	# Refer to Figure 14.21 from the textbook (7th edition) starting from "else begin /* Redistribution: ..."
	...
	def redistributeWithBlock(self, otherBlock, kprime):
		...
		if self.isUnderfull():
			...
			if self.isLeaf:
				temp_key = otherBlock.keysAndPointers.pop(-2)
				temp_ptr = otherBlock.keysAndPointers.pop(-2)

				self.keysAndPointers.insert(0, temp_key)
				self.keysAndPointers.insert(0, temp_ptr)

				temp_ptr.getBlock().parent = self
				return temp_key
			...
			# Case 2: successor is underfull and is not leaf
				...
				temp_ptr = otherBlock.keysAndPointers.pop()
				temp_key = otherBlock.keysAndPointers.pop()
				...
				temp_ptr.getBlock().parent = self
				return temp_key
			# "borrow" one pointer from predecessor, moving last key up to parent, moving one key down to successor
			...
			if otherBlock.isLeaf:
				other_ptr = self.keysAndPointers.pop(0)
				other_key = self.keysAndPointers.pop(0)
				

				otherBlock.keysAndPointers.insert(-1,other_ptr)
				otherBlock.keysAndPointers.insert(-1,other_key)

				other_ptr.getBlock().parent = otherBlock
			...
			# Case 4: predecessor is underfull and is not leaf
				...
				otherBlock.keysAndPointers.append(kprime)
				otherBlock.keysAndPointers.append(other_ptr)
	...
	# The following roughly (and partially) implements the algorithm from Figure 14.21 (7th edition)
